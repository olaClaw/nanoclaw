...
**Verify the bind interface (container reachability).** Agent containers reach the gateway over the Docker bridge through `host.docker.internal`, so a server bound only to loopback boots cleanly on the host while every credentialed call from containers fails at the proxy:
